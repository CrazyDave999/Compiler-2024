    ...
    for node in ir {
        match &node {
            IRNode::FuncBegin(_, name, args) => {
                ...
                for (i, (ty, name)) in args.iter().enumerate() {
                    if (i as i32) < a_num {
                        ...
                        func_inner.push(IRNode::ArgLoad(
                            ty.clone(),
                            name.clone(),
                            (i as i32 - a_num) * 4,
                        ));
            ...
            IRNode::FuncEnd => {
                ...
                alloc_res
                    .color
                    .insert(func_name.clone().unwrap(), Box::new(alloc.get_color()));
                alloc_res
                    .spill_temps
                    .insert(func_name.clone().unwrap(), Box::new(alloc.get_spill_vars()));
                    ...
                    match &node {
                        IRNode::Call(res, res_ty, name, args) => {
                            ...
                            for (i, (ty, arg)) in args.iter().enumerate() {
                                if (i as i32) < a_num {
                                    func_inner.push(IRNode::Move(
                                        ty.clone(),
                                        format!("%a{}", i),
                                        arg.clone(),
                                    ));
                                } else {
                                    func_inner.push(IRNode::ArgStore(
                                        ty.clone(),
                                        arg.clone(),
                                        (i as i32 - a_num) * 4,
                                    ));
                            ...
                            func_inner.push(IRNode::CallerRecover(name.clone(), Vec::new()));
                            // 预留恢复命令
                        }
                        IRNode::Ret(ty, res) => {
                            if let Some(res) = res {
